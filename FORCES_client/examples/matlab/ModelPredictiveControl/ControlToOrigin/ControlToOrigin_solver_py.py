...
a Python wrapper for a fast solver generated by FORCESPRO v5.1.0
   ...
   OUTPUT = ControlToOrigin_solver_py.ControlToOrigin_solver_solve(PARAMS) solves a multistage problem
   subject to the parameters supplied in the following dictionary:
       PARAMS['minusA_times_x0'] - column vector of length 2

   OUTPUT returns the values of the last iteration of the solver where
       OUTPUT['u0'] - column vector of size 1

   [OUTPUT, EXITFLAG] = ControlToOrigin_solver_py.ControlToOrigin_solver_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = ControlToOrigin_solver_py.ControlToOrigin_solver_solve(PARAMS) returns 
...
#_lib = ctypes.CDLL(os.path.join(os.getcwd(),'ControlToOrigin_solver/lib/ControlToOrigin_solver.so')) 
try:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'ControlToOrigin_solver/lib/ControlToOrigin_solver.so'))
    csolver = getattr(_lib,'ControlToOrigin_solver_solve')
except:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'ControlToOrigin_solver/lib/libControlToOrigin_solver.so'))
    csolver = getattr(_lib,'ControlToOrigin_solver_solve')

class ControlToOrigin_solver_params_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('minusA_times_x0', ctypes.c_double * 2),
]

ControlToOrigin_solver_params = {'minusA_times_x0' : np.array([]),
}
params = {'minusA_times_x0' : np.array([]),
}
ControlToOrigin_solver_params_types = {'minusA_times_x0' : np.float64,
}

class ControlToOrigin_solver_outputs_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
ControlToOrigin_solver_outputs = {'u0' : np.array([]),
}


class ControlToOrigin_solver_info(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
csolver.argtypes = ( ctypes.POINTER(ControlToOrigin_solver_params_ctypes), ctypes.POINTER(ControlToOrigin_solver_outputs_ctypes), ctypes.POINTER(ControlToOrigin_solver_info), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

def ControlToOrigin_solver_solve(params_arg):
    '''
a Python wrapper for a fast solver generated by FORCESPRO v5.1.0
 ...
 See also COPYING
    ...
    # convert parameters
    params_py = ControlToOrigin_solver_params_ctypes()
    for par in params_arg:
        try:
            ...
            if isinstance(getattr(params_py, par), ctypes.Array):
                params_arg[par] = np.require(params_arg[par], dtype=ControlToOrigin_solver_params_types[par], requirements='F')
    ...
    outputs_py = ControlToOrigin_solver_outputs_ctypes()
    info_py = ControlToOrigin_solver_info()
    if sys.version_info.major == 2:
        ...
        try:
            PyFile_AsFile.restype = ctypes.POINTER(FILE)
            exitflag = _lib.ControlToOrigin_solver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), PyFile_AsFile(fp)  )
        ...
        try:
            if sys.platform.startswith('win'):
                exitflag = _lib.ControlToOrigin_solver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), None )
            else:
                exitflag = _lib.ControlToOrigin_solver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), fp )
            libc.fclose(fp)
    ...
    # convert outputs
    for out in ControlToOrigin_solver_outputs:
        ControlToOrigin_solver_outputs[out] = npct.as_array(getattr(outputs_py,out))

    return ControlToOrigin_solver_outputs,int(exitflag),info_py

solve = ControlToOrigin_solver_solve
