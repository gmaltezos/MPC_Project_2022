classdef forcesNLPBuildable < coder.ExternalDependency
...
%   Copyright 2019-2021 The MathWorks, Inc.
    ...
    methods (Static)
        ...
        function updateBuildInfo(buildInfo, cfg) %#ok<INUSD>
            % FORCESPRO path
            ...
            isDSpaceAutoBox = strcmpi(cfg.getConfigProp('SystemTargetFile'), 'rti1007.tlc');
            isDSpaceMicroLabBox = strcmpi(cfg.getConfigProp('SystemTargetFile'), 'rti1202.tlc');
            isDSpaceMABII = strcmpi(cfg.getConfigProp('SystemTargetFile'), 'rti1401.tlc');
            isDSpaceMABXIIIorSCALEXIO = strcmpi(cfg.getConfigProp('SystemTargetFile'), 'dsrt.tlc');
            isDSpace = isDSpaceAutoBox || isDSpaceMicroLabBox || isDSpaceMABII || isDSpaceMABXIIIorSCALEXIO;
            ...
            isTargetPlatform = isTargetPlatform || isDSpace || isSpeedgoat || isSpeedgoatQNX;
            ...
            elseif( ismac || isunix || isMinGW || isSpeedgoatQNX || isDSpace )
        ...
        function [output,status,SolverInfo] = forcesNLP(coredata,onlinedata,x,lastMV,md,ref,mvTarget,para)
            %#codegen
            ...
            params = forcesnlmpcGetParamValues(coredata,onlinedata,x,lastMV,md,ref,mvTarget,para);
            ...
            output = struct('MVopt',zeros(coredata.p*coredata.nmv,1),'Xopt',zeros(coredata.p*coredata.nx,1));
